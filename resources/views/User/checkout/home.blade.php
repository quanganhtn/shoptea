        ...
        <div class="container">
            ...
            <div class="news-grid mt-2">
                @for($i=0; $i<3; $i++)
                    @php
                        $c = $cards[$i] ?? [];
                        $img = $c['image'] ?? 'images/nentra.jpg';
                        $text = $c['text'] ?? 'Chưa có nội dung';
                        $link = $c['link'] ?? '#';
                    @endphp

                    <div class="news-card">
                        <img src="{{ asset($img) }}" alt="news" class="news-card__img">
                        <div class="news-card__body">
                            <div class="news-card__text">
                                {{ Str::limit($text, 90, '...') }}
                            </div>

                            <a href="{{ $link }}" target="_blank" class="btn btn-dark news-card__btn">Xem thêm</a>
                        </div>
                    </div>
                @endfor
            </div>
